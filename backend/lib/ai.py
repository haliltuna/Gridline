...
RENDER_SCALE = 3.5  # ~270 DPI — lets Opus 4.7 actually read the printed callout text
...
# ---------------------------------------------------------------------------
# Pure logic — takes `profile`, the merged wizard settings, everywhere it matters
# ---------------------------------------------------------------------------
...
def _rate_for(floor_type: str, profile: dict[str, Any] | None, fallback: float) -> float:
    """The installer's own $/sf for this floor type, else the industry fallback."""
    if not profile:
...
def _waste_for(floor_type: str, profile: dict[str, Any] | None, fallback: float) -> float:
    """The installer's waste % for this floor type, else the industry fallback."""
    if not profile:
        return float(fallback)
...
def _install_method_for(floor_type: str, profile: dict[str, Any] | None) -> str:
    """Which install method applies to this floor type given the profile.

    If the profile is "mixed", we pick the first valid method the floor type supports
    that also uses adhesive — that matches the old behaviour and keeps quotes simple.
    If the profile's method isn't valid for this floor type, we fall back to the
    floor type's primary method.
    """
    ...
    if method == "mixed":
        # Prefer glued if the floor type supports it — matches the old behaviour where
        # "mixed" was the de-facto glued default.
...
def _adhesive_visible(floor_type: str, profile: dict[str, Any] | None) -> bool:
    """Whether adhesive should appear at all on this line, given the profile."""
...
def _scope_for(raw_scope: str | None, profile: dict[str, Any] | None) -> str:
    """Decide the pricing scope for a line.

    Precedence: what the AI explicitly said > what the wizard profile says > "supply_install".
    """
...
def build_line(raw: dict[str, Any], job_id: str, labor_rate: float,
    ...
    # Waste precedence: what the AI read on the sheet > the installer's own override in the
    # legacy settings.waste_overrides > the wizard's waste_pct > the industry default.
    ...
    # Material rate: AI-read spec price > the installer's wizard rate > the industry default.
...
def build_accessory_lines(
    ...
    """Turn the AI's counts into priced accessory lines.

    Transition strips honour the wizard's `transition_rule`:
      * "on_flooring_change" (default) — count each doorway once, but weight the estimate to
        reflect that roughly a third of doorways actually change flooring. This keeps the line
        conservative without dropping the scope.
      * "every_door" — use the AI's raw door count as-is.
    """
    ...
    # Fraction of doorways that actually involve a flooring change — industry rule of thumb.
    # When the installer bids "every door", we don't scale.
    ...
    for g in groups:
        ...
        for kind, key, room in (
            ...
            # Apply the wizard's transition rule
            ...
            if kind == "transition" and transition_rule == "on_flooring_change":
                where += " · filtered to"""Blueprint reading engine: PDF pages -> high-res PNG -> Claude Opus vision -> takeoff lines.
